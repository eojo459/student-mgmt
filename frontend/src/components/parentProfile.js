...
export default function ParentProfile() {
	...
	const getParent = () => {
		...
		axios({
			method: "get",
			// todo
			url: process.env.REACT_APP_API + "parent/" + id,
			headers: {
				Authorization: "Bearer " + JSON.parse(localStorage.getItem("authTokens"))["access"],
			},
		})
	...
	const addStudentToParent = () => {
		...
		axios({
			method: "POST",
			url: process.env.REACT_APP_API + "parent/" + id + "/students/",
			headers: {
				Authorization: "Bearer " + JSON.parse(localStorage.getItem("authTokens"))["access"],
			},
			data: data,
		...
		<React.Fragment>
			<FormControl>
				<Grid container spacing={1.5}>
						...
						<React.Fragment key={student.studentId}>
							<Grid item xs={12} sm={6} md={4}>
								...
								<Typography variant="body1">
									Course IDs: {student.courses ? student.courses.join(", ") : "N/A"}
								</Typography>
								{/* view profile */}
								<Button variant="contained" onClick={() => navigate(`/student-profile/${student.studentId}`)}>
									Profile
								</Button>
							</Grid>
					...
					<Grid item xs={12} sm={12}>
						<br />
						<Button variant="contained" fullWidth onClick={addStudentToParent}>
							Add Existing Child
						</Button>
					</Grid>
					{/* Notifications */}
